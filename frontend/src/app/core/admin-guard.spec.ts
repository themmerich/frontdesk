...
describe('adminGuard', () => {
  ...
  it('lets admins pass', async () => {
    currentUser.set({ username: 'anna', displayName: 'Anna', role: 'admin', tenantName: 'Musterfirma GmbH', hasAvatar: false });

    expect(await runGuard()).toBe(true);
  });

  it('sends regular users back to the start page', async () => {
    currentUser.set({ username: 'uwe', displayName: 'Uwe', role: 'user', tenantName: 'Musterfirma GmbH', hasAvatar: false });
